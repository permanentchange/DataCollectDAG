...
3. 从配置声明的 ROS1 topic 接入图像、点云、IMU、NMEA、里程计数据。
...
工具不自动扫描并订阅 ROS graph 或 rosbag 中的所有 topic。
...
若配置中使用 `bdstar/string`、`igv_msgs/location` 等自定义 ROS message，目标机器必须已安装或编译对应 ROS package，并在启动前执行工作空间环境脚本，例如：
...
```text
tool_state：工具运行状态。
recent_session_status：最近 session 状态。
```
...
工具第一阶段应支持从配置声明的 ROS1 topic 接入以下数据。
...
第一版应支持 NMEA 字符串类 topic，具体 ROS message type 由配置文件声明。

第一版真实 rosbag 示例 topic：

```text
/bdstar/nmea_sentence
...
工具应对配置声明的外部输入数据进行 session 级有限缓存。

缓存只在当前 session RUNNING 期间有效。

IDLE 状态不缓存输入数据。

当前 session 只接收 session start 后到达的数据。
...
receive_timestamp 与 session_start_time 均使用工具所在机器的系统时间。

source_timestamp 可用于同步匹配，不用于判断数据是否属于当前 session。

不满足 receive 时间边界的数据应丢弃，并记录 `drop_reason = before_session_start`。

工具只订阅配置文件 `ros.topics` 中声明的 topic。

未配置 topic 默认忽略，不缓存、不进入 DAG、不计入 `received_messages`。
...
图像解码、图像编码、点云解析、IMU 解析、NMEA 解析由具体节点按需执行。
...
8. context key 不存在时返回 missing 错误。
9. context key 存在多个 producer 且未指定 producer 时返回 ambiguous 错误。
...
节点主动返回 NodeResult 时，DAG 执行器直接使用该结果。节点抛异常时，DAG 执行器根据 on_error 策略转换为 NodeResult。
...
所有 wait 操作必须有超时时间。第一阶段不要求支持定位、里程计、IMU 或 NMEA 插值。
...
保存节点返回 OK 前，不应设置 save_result.saved=true。

保存节点返回 OK 只表示节点执行成功，不等同于样本已保存。

只有 save_result.saved=true 且 DAG 到达 end，才计入 samples_saved。
...
samples_saved 只统计 save_result.saved=true 且 DAG 到达 end 的样本。
...
节点长期不响应取消会阻塞 stop / replace，属于节点实现错误。
