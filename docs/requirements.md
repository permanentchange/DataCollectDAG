...
不满足 receive 时间边界的数据应丢弃，并记录 drop_reason 为 before_session_start。
...
所有 wait 操作必须有超时时间。第一阶段不要求支持定位或里程计插值。
...
DAG 执行器只在 end node 到达后统计 samples_saved。
...
samples_saved 只统计保存节点明确返回 OK 且 save_result.saved=true 的样本。
